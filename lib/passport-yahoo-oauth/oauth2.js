...
function Strategy(options, verify) {
    ...
    options = options || {};
    options.authorizationURL = options.authorizationURL || 'https://api.login.yahoo.com/oauth/v2/request_auth';
    options.tokenURL = options.tokenURL || 'https://api.login.yahoo.com/oauth/v2/get_token';
...
Strategy.prototype.userProfile = function (accessToken, done) {
    ...
    this._oauth2.get(this._userGUIDUrl, accessToken, function (err, body) {
        ...
        self._oauth2.get(url, accessToken, function(profileError, profileBody) {
            ...
            try {
                json = JSON.parse(profileBody).profile;
            } catch (ex) {
            ...
            var profile = {
                ...
                emails: json.emails && json.emails.map(function (email) {
                    ...
                    obj.type = email.primary ? 'account' : email.type || 'unknown';
                    return obj;
                }) || [],
                photos: [{
